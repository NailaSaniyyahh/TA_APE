...
input_file  = BASE_DIR / "../CODE-DATASET/v10test_queries_final.json"
...
def get_candidates(query: str) -> list[dict]:
    ...
    query_for_fuzzy = " ".join(query_lower.split()[:5])
    body = {
        "suggest": {
            "suggest_by_title": {
                "prefix": query_lower,
    ...
    if len(result) < 10:
        shortage = 10 - len(result)
        Fbody = {
            "query": {
                "bool": {
                    "should": [
                        {"match": {"title":  {"query": query_for_fuzzy, "fuzziness": "1"}}},
                        {"match": {"author": {"query": query_for_fuzzy, "fuzziness": "1"}}}
                    ],
        ...
        fuzzy_resp = client.search(index=INDEX_NAME, body=Fbody)
        for hit in fuzzy_resp.get("hits", {}).get("hits", []):
            if len(result) >= 10:
                break
            src   = hit["_source"]
            title = src.get("title", "")
...
def _base(q, alpha):
    return {"query_id": q["query_id"], "query_text": q["query_text"],
            "query_type": q.get("mode_qac", "UNKNOWN"), "title": q.get("title",""),
